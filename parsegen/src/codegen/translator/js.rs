...
impl Translator for Js {
    fn translate_step(&mut self, bc: &ByteCode, w: &mut dyn Write) -> Result<(), Error> {
        match bc {
            ...
            ByteCode::Eq(dest, lhs, rhs) => writeln!(w, "{} = {} === {};", dest.name, lhs, rhs)?,
            ByteCode::And(dest, lhs, rhs) => writeln!(w, "{} = {} && {};", dest.name, lhs, rhs)?,
            ...
            ByteCode::IndexSet(arr, idx, val) => writeln!(w, "{}[{}] = {};", arr.name, idx, val)?,
...
pub mod tests {
    ...
    impl Translator for NodeRunner {
        fn translate_step(&mut self, bc: &ByteCode, w: &mut dyn Write) -> Result<(), Error> {
            Js.translate_step(bc, w)
        }
    }
